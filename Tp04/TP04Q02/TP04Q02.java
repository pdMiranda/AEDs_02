...
public class TP04Q02{
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] entradaPt2_split = new String[2];
        int intInsert;

        ArrayList<Game> games = new ArrayList<Game>();
        LinkedList<Game> listaGames = new LinkedList<Game>();
        LinkedList<Game> removeGames = new LinkedList<Game>();
        int entradaLista = 0;
        int entrada = 0;
        int entradaR = 0;
        ...
        for(int i = 0; i < numEntrada_str; i++){
            if(entradaPt2[i].charAt(0) == 'I'){
                entradaPt2_split = entradaPt2[i].split(" ");
                intInsert = Integer.parseInt(entradaPt2_split[1]);
                for(int j = 0; j < entrada; j++){
                    if(games.get(j).getAppId() == intInsert){
                        listaGames.add(games.get(j));
                        entradaLista++;
                    }
                }
            }
            else if(entradaPt2[i].charAt(0) == 'R'){
                removeGames.add(listaGames.get(entradaLista - 1));
                listaGames.remove(entradaLista -1 );
                entradaLista--;
                entradaR++;
            }
        }

        for(int i = 0; i < entradaR; i++){
            System.out.println("(R) "+ removeGames.get(i).getName());
        }
        for(int i = 0; i < entradaLista; i++){
            System.out.println("[" + i + "] " + listaGames.get(i).print());
