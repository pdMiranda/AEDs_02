...
void game_print(Game *game) {
    ...
    if(game -> count_genres == 1){
        char g[50];
        strcpy(g, game -> genres[0]);
        int tam = strlen(g);
        g[tam - 2] = '\0';
...
CelulaDupla* CelulaPosicao(int pos){
...
int posMes(CelulaDupla a, char* arranjo[]){
    int i = 0;
    int resp;
    ...
    while (i < 12 && !find)
    {
        int n = 0;
        while(!find && n < 12){
            if(*arranjo[n] == a.elemento.release_date.month){
                resp = n + 1;
                find = true;
                n = 12;
            }
...
bool antes(CelulaDupla* a, CelulaDupla* b){
    bool antes = false;
    char* mes[] = {"Jan", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
    if(a->elemento.release_date.year == b->elemento.release_date.year){
        if(posMes(*a,mes) < posMes(*b,mes)) antes = true;
    }
    else if(a->elemento.release_date.year < b->elemento.release_date.year){
        antes = true;
    }
...
void ordenar(int esq, int dir) {
    ...
    int pivo = CelulaPosicao((dir+esq)/2)->elemento.release_date.year;

    while (i <= j) {
        while (CelulaPosicao(i)->elemento.release_date.year < pivo) i++;
        while (CelulaPosicao(j)->elemento.release_date.year > pivo) j--;
        
        if (i <= j) {             
            if(antes(CelulaPosicao(i), CelulaPosicao(j))){
                Game tmp = CelulaPosicao(i)->elemento;
                CelulaPosicao(i)->elemento = CelulaPosicao(j)->elemento;
                CelulaPosicao(j)->elemento = tmp;
            ...
            */
            ...
            /*else if(cmpData == 0){
                int cmpNome = strcmp(CelulaPosicao(i)->elemento.name, desempate);
                if(cmpNome > 0){
                    Game tmp = CelulaPosicao(i)->elemento;
                    CelulaPosicao(i)->elemento = CelulaPosicao(j)->elemento;
                    CelulaPosicao(j)->elemento = tmp;
...
int main() {
    ...
    while(true) {
        ...
        if(found.app_id != -1) list_insert(found);
        quant++;
