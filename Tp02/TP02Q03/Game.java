...
class Game {
    ...
    public static boolean hasInt(int[] lista, int chave, int tamanho){
        boolean has = false;
        int n = 0;
        while(!has && n < tamanho){
            if(lista[n] == chave){
    ...
    public static int hasString(ArrayList<Game> games, String chave, int tamanho){
        int has = 0;
        int n = 0;
        String nome;
        boolean found = false;
        while(!found && n < tamanho){
            nome = games.get(n).getName();
            if(nome.compareTo(chave) == 0){
                found = true;
                has = games.get(n).getAppId();
            }
            count++;
            n++;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        do{
            ...
            numEntrada_str++;
        }while(!isFim(entradaPt2[numEntrada_str - 1]));
        numEntrada_str--;

        for(int i = 0; i < numEntrada_str; i++){
            resp[i] = hasString(games, entradaPt2[i], numEntrada);
        }

        for(int i = 0; i < numEntrada_str; i++){
            if(hasInt(entradaPt1_int, resp[i],numEntrada_int )){
        ...
        logFile.write("761531\t" + time/1000 + "\t" + count);
