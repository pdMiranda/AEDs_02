...
public class TP02Q07 {
    ...
    public static void main(String[] args) throws Exception{
        ...
        ArrayList<Game> gamesOficial = new ArrayList<Game>();
        ArrayList<Game> gamesRemove = new ArrayList<Game>();
        ArrayList<Game> gamesInsert = new ArrayList<Game>();
        int numListaOficial = 0;

        String[] entradaPt1 = new String[1000];
        int[] entradaPt1_int = new int[1000];
        int numEntrada_int = 0;

        String[] entradaPt2 = new String[1000];
        int numEntrada_str = 0;
        ...
        do{
            entradaPt1[numEntrada_int] = sc.nextLine();
            numEntrada_int++;
        }while(!isFim(entradaPt1[numEntrada_int - 1]));
        numEntrada_int--;

        for(int i = 0; i < numEntrada_int; i++){
            entradaPt1_int[i] = Integer.parseInt(entradaPt1[i]);
        }

        do{
            entradaPt2[numEntrada_str] = sc.nextLine();
            numEntrada_str++;
        }while(numEntrada_str <( Integer.parseInt(entradaPt2[0]) + 1));
        ...
        for(int i = 0; i < gamesLista.size(); i++){
            if(numListaOficial >= 5){
                gamesOficial.add(gamesLista.get(i));
                gamesOficial.remove(0);
            }
            else{
                gamesOficial.add(gamesLista.get(i));
                numListaOficial++;
        ...
        for(int i = 0; i < numEntrada_str; i++){
            if(entradaPt2[i].charAt(0) == 'I'){
                Split_Str = entradaPt2[i].split(" ");
                int num = Integer.parseInt(Split_Str[1]);
                for(int j = 0; j < games.size(); j++){
                    if(games.get(j).getAppId() == num){
                        gamesLista.add(games.get(j));
                        gamesInsert.add(games.get(j));
                        if(numListaOficial >= 5){
                            gamesOficial.add(gamesLista.get(gamesLista.size() - 1));
                            gamesOficial.remove(0);
                        }
                        else{
                            gamesOficial.add(gamesLista.get(gamesLista.size() - 1));
                ...
                gamesRemove.add(gamesOficial.get(0));
                gamesOficial.remove(0);
                numListaOficial--;
                gamesLista.remove(gamesLista.size() - 1);
            }
        }

        for(int i = 0; i < gamesLista.size(); i++){
            System.out.println(gamesLista.get(i).getAvgPlaytime());
        }
        ...
        for(int i = 0; i < numEntrada_str; i++ ){
            if(entradaPt2[i].charAt(0) == 'R'){
                ...
                System.out.println("(R) " + gamesRemove.get(aux1).getName());
                aux1++;
            }
            else if(entradaPt2[i].charAt(0) == 'I'){
                System.out.println(entradaPt2[i]);
                System.out.println(Math.round(gamesInsert.get(aux2).getAvgPlaytime()));
                aux2++;
            }
        }

        for(int i = 0; i < gamesOficial.size(); i++){
            System.out.println("[" + i + "] " + gamesOficial.get(i).print());
        }
