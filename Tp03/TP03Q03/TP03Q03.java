...
public class TP03Q03 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        do{
            ...
            numEntrada++;
        }while(!isFim(entrada[numEntrada - 1]));
        numEntrada--;

        int[] entrada_int = new int[numEntrada];

        for(int i = 0; i < numEntrada; i++){
            entrada_int[i] = Integer.parseInt(entrada[i]);
        }
        ...
        for(int i = 0; i < numEntrada; i++){
            ...
            while(!has && n < games.size()){
                if(games.get(n).getAppId() == entrada_int[i]){
        ...
        for(int i = 1; i < gamesLista.size(); i++){
            Game tmp = gamesLista.get(i);
            int j = i - 1;

            while((j >= 0)&&(gamesLista.get(j).getAppId() > tmp.getAppId())){
                gamesLista.set(j + 1, gamesLista.get(j));
                j--;
                count1++;
            }
            gamesLista.set(j + 1, tmp);
            count2++;
        }

        for(int i = 0; i < gamesLista.size(); i++){
            gamesLista.get(i).print();
        }
