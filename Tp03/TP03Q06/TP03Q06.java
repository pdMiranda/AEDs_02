...
public class TP03Q06 {
    ...
    public static void quicksort(Game[] array, int esq, int dir){
        int i = esq, j = dir;
        Game pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getReleaseDate().before(pivo.getReleaseDate())) i++; count1++;
            while (array[j].getReleaseDate().after(pivo.getReleaseDate())) j--; count1++;
            if (i <= j) {
                swap(array,i, j);
                i++;
                j--;
    ...
    public static void main(String[] args) throws Exception{
        ...
        quicksort(gamesList, 0, numEntrada - 1);
