    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int i = 0; i < NumEntrada; i++){
            ...
            while(!has && n < games.size()){
                if(entrada_int[i] == games.get(n).getAppId()){
                    gamesList.add(games.get(n));
                    has = true;
                    n = games.size();
                }
                n++;
        ...
        for(int i = 0; i < gamesList.size() - 1; i++){
            ...
            for(int j = i + 1;j < gamesList.size(); j++){
                count1++;
                if(compareStrings(gamesList.get(menor).getName(),gamesList.get(j).getName()) > 0){
                    menor = j;
                    count2++;
                }
            }
            Game temp = gamesList.get(menor);
            gamesList.set(menor, gamesList.get(i));
            gamesList.set(i, temp);
        }
